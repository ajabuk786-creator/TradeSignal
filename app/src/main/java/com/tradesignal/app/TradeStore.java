...
public final class TradeStore {
    ...
    public static final class ActiveTrade {
        ...
        public ActiveTrade(String marketId,String signalId,String side,String setupLabel,double entry,double stopLoss,double takeProfit,
                           double originalRisk,double qty,double trailingTrigger,int confidence,long openedAt,boolean trailingActive){
    ...
    public static ActiveTrade load(Context ctx,String marketId){
        ...
        return new ActiveTrade(marketId,p.getString(key(marketId,"signalId"),""),p.getString(key(marketId,"side"),""),
                p.getString(key(marketId,"setup"),""),getDouble(p,key(marketId,"entry"),0),getDouble(p,key(marketId,"sl"),0),
                getDouble(p,key(marketId,"tp"),0),getDouble(p,key(marketId,"risk"),0),getDouble(p,key(marketId,"qty"),0),
                getDouble(p,key(marketId,"trigger"),0),p.getInt(key(marketId,"confidence"),0),p.getLong(key(marketId,"opened"),0),
    ...
    public static boolean openFromRecord(Context ctx,SignalHistoryStore.Record r,long now){
        ...
        SharedPreferences p=ctx.getSharedPreferences(PREFS,Context.MODE_PRIVATE);
        p.edit().putBoolean(key(r.marketId,"active"),true).putString(key(r.marketId,"signalId"),r.id)
                .putString(key(r.marketId,"side"),r.side).putString(key(r.marketId,"setup"),r.confirmation)
                .putLong(key(r.marketId,"entry"),Double.doubleToRawLongBits(r.entry))
                .putLong(key(r.marketId,"sl"),Double.doubleToRawLongBits(r.stopLoss))
                .putLong(key(r.marketId,"tp"),Double.doubleToRawLongBits(r.takeProfit))
                .putLong(key(r.marketId,"risk"),Double.doubleToRawLongBits(r.riskPerUnit))
                .putLong(key(r.marketId,"qty"),Double.doubleToRawLongBits(r.qty))
                .putLong(key(r.marketId,"trigger"),Double.doubleToRawLongBits(r.trailingTrigger))
                .putInt(key(r.marketId,"confidence"),r.confidence).putLong(key(r.marketId,"opened"),now)
                .putBoolean(key(r.marketId,"trail"),false).apply();
        return true;
    }

    public static boolean inCooldown(Context ctx,String marketId,long now){
        return now<ctx.getSharedPreferences(PREFS,Context.MODE_PRIVATE).getLong(key(marketId,"cooldown"),0L);
    }
    ...
    public static Update update(Context ctx,String marketId,List<SignalEngine.Candle> m5){
        ...
        if(sl!=t.stopLoss||trail!=t.trailingActive){
            ...
            t=new ActiveTrade(t.marketId,t.signalId,t.side,t.setupLabel,t.entry,sl,t.takeProfit,t.originalRisk,t.qty,t.trailingTrigger,t.confidence,t.openedAt,trail);
        }
    ...
    private static Update close(Context ctx,ActiveTrade t,double exit,String reason){
        ...
        SharedPreferences p=ctx.getSharedPreferences(PREFS,Context.MODE_PRIVATE);
        p.edit().putBoolean(key(t.marketId,"active"),false).putString(key(t.marketId,"last_result"),result)
                .putLong(key(t.marketId,"cooldown"),System.currentTimeMillis()+COOLDOWN_MS).apply();
        SignalHistoryStore.markTakenOutcome(ctx,t.signalId,reason,exit);
        return new Update(null,result,true,exit);
    }

    private static double atr(List<SignalEngine.Candle> c,int period){
        if(c.size()<=period)return Double.NaN;int start=Math.max(1,c.size()-period);double sum=0;int n=0;
        for(int i=start;i<c.size();i++){SignalEngine.Candle cur=c.get(i),prev=c.get(i-1);double tr=Math.max(cur.high-cur.low,Math.max(Math.abs(cur.high-prev.close),Math.abs(cur.low-prev.close)));sum+=tr;n++;}
