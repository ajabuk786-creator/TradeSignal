...
public final class SignalHistoryStore {
    ...
    private static final long SAME_STRUCTURE_SUPPRESS_MS=2L*60L*60L*1000L;
    ...
    public static synchronized boolean add(Context ctx,MarketDataClient.Market market,SignalEngine.Signal s,long detectedAt){
        ...
        // One idea, one signal: do not fire every 5m candle while the same structure/setup is still active.
        for(Record r:all){
            if(!r.marketId.equals(market.id)||!r.side.equals(s.side))continue;
            long age=Math.abs(s.candleCloseTime-r.signalTime);
            boolean sameStructure=!s.structureKey.isEmpty()&&s.structureKey.equals(r.structureKey)&&age<SAME_STRUCTURE_SUPPRESS_MS;
            boolean sameSetup=s.confirmationLabel.equals(r.confirmation)&&age<SAME_SETUP_SUPPRESS_MS;
    ...
    public static synchronized int updateUntakenOutcomes(Context ctx,String marketId,List<SignalEngine.Candle> m5){
        ...
        for(Record r:all){
            ...
            long end=r.signalTime+PAPER_TRACK_MS;double risk=Math.max(r.riskPerUnit,1e-12);
            for(SignalEngine.Candle c:m5){
                if(c.closeTime<=r.signalTime||c.openTime>end)continue;
                double fav,adv;if("BUY".equals(r.side)){fav=(c.high-r.entry)/risk;adv=(r.entry-c.low)/risk;}else{fav=(r.entry-c.low)/risk;adv=(c.high-r.entry)/risk;}
                if(Double.isFinite(fav))r.maxFavorableR=Math.max(r.maxFavorableR,fav);if(Double.isFinite(adv))r.maxAdverseR=Math.max(r.maxAdverseR,adv);
    ...
    private static List<Record> load(Context ctx){SharedPreferences p=ctx.getSharedPreferences(PREFS,Context.MODE_PRIVATE);String raw=p.getString(KEY,"[]");ArrayList<Record> out=new ArrayList<>();try{JSONArray a=new JSONArray(raw);for(int i=0;i<a.length();i++)out.add(fromJson(a.getJSONObject(i)));}catch(Exception ignored){}Collections.sort(out,(a,b)->Long.compare(b.signalTime,a.signalTime));return out;}
    ...
    private static JSONObject toJson(Record r){JSONObject o=new JSONObject();try{o.put("id",r.id).put("marketId",r.marketId).put("symbol",r.symbol).put("side",r.side).put("entry",r.entry).put("sl",r.stopLoss).put("tp",r.takeProfit).put("risk",r.riskPerUnit).put("qty",r.qty).put("trigger",r.trailingTrigger).put("rr",r.riskReward).put("confidence",r.confidence).put("quality",r.qualityScore).put("setup",r.setupConfidence).put("triggerConfidence",r.triggerConfidence).put("confirmation",r.confirmation).put("amd",r.amdState).put("structureKey",r.structureKey).put("regime",r.regime).put("engineVersion",r.engineVersion).put("signalTime",r.signalTime).put("detectedAt",r.detectedAt).put("takenAt",r.takenAt).put("status",r.status).put("outcome",r.outcome).put("outcomePrice",r.outcomePrice).put("maxFavorableR",r.maxFavorableR).put("maxAdverseR",r.maxAdverseR);}catch(JSONException ignored){}return o;}

    private static Record fromJson(JSONObject o){Record r=new Record();r.id=o.optString("id","");r.marketId=o.optString("marketId","");r.symbol=o.optString("symbol",r.marketId);r.side=o.optString("side","");r.entry=o.optDouble("entry",0);r.stopLoss=o.optDouble("sl",0);r.takeProfit=o.optDouble("tp",0);r.riskPerUnit=o.optDouble("risk",Math.abs(r.entry-r.stopLoss));r.qty=o.optDouble("qty",0);r.trailingTrigger=o.optDouble("trigger",0);r.riskReward=o.optDouble("rr",1.3);r.confidence=o.optInt("confidence",0);r.qualityScore=o.optInt("quality",Math.max(0,Math.min(10,(int)Math.round(r.confidence/10.0))));r.setupConfidence=o.optInt("setup",0);r.triggerConfidence=o.optInt("triggerConfidence",0);r.confirmation=o.optString("confirmation","");r.amdState=o.optString("amd","");r.structureKey=o.optString("structureKey","");r.regime=o.optString("regime","");r.engineVersion=o.optString("engineVersion","v4-legacy");r.signalTime=o.optLong("signalTime",0);r.detectedAt=o.optLong("detectedAt",0);r.takenAt=o.optLong("takenAt",0);r.status=o.optString("status","NEW");r.outcome=o.optString("outcome","");r.outcomePrice=o.optDouble("outcomePrice",0);r.maxFavorableR=o.optDouble("maxFavorableR",0);r.maxAdverseR=o.optDouble("maxAdverseR",0);return r;}
}
